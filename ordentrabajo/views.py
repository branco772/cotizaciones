...
def registroOrden(request):
    materiales=Materiales.objects.all()
    return render(request, "registroOrden.html", {'materiales':materiales})
ULTIMA_FACTURA = 0
def enviarformularioOrden(request):
    ...
    if request.method=="POST":
        ...
        codigo="OT"+ fecha_actua2+"-{}".format(ULTIMA_FACTURA)
        contadorPosicion=0
        totalSuma=0
        ...
        if datos_tabla:
            ...
            for item in data:
                ...
                des=totalSuma*(int(descuento)/100)
                precioDescuento=totalSuma-des
                if int(cantidad)< material.stock:
                    messages.add_message(request, level=messages.WARNING, message= f"¡Alerta! El material {material.nombrematerial} sobrepaso el STOCK de: {material.stock}") 
                    print(f"¡Alerta! El material {material.nombrematerial} tiene un stock {material.stock}")
                    mensaje_alerta=f"¡Alerta! El material {material.nombrematerial} sobrepaso el STOCK de: {material.stock}"
                #DISMINUIR EL STOCK
                print(material.stock, int(cantidad))
                material.cantidad-=int(cantidad)
                if material.cantidad==0 or material.cantidad<=0:
                    material.cantidad=0
                material.save()
                usuario = Orden(cliente=cliente, direccion=direccion, telefono=telefono, ciudad=ciudad, descripcion=descripcion, cantidad=cantidad, valorunitario=valorunitario, valortotal=valortotal, descuento=descuento, codigo=codigo, fecha=date.today(), descripcionservicio=descripcionServicio, serviciototal=totalSuma, preciodescuento=precioDescuento, materialutilizado=material, cantidadutilizado=int(cantidad))
                usuario.save()
        else:
            print('NO HAY DATOS')
        #ot2305-01 y ct
        ...
        p.drawString(240, 810, "ORDEN DE TRABAJO")
        #tabla CERO
        tablacero= [['COTIZACIÓN'],
                    ['OT'+ fecha_actua2+"-{}".format(ULTIMA_FACTURA)],
                    ['{}'.format(fecha_actual)]]
        ...
        p.save()    
        return response
    return render(request, 'registroOrden.html', {'mensaje_alerta':mensaje_alerta})

def buscarcotizaciones(request):
    if request.method == 'GET':
        codigo = request.GET.get('numero_factura')
        cotizaciones = Cliente.objects.filter(codigo=codigo)
        return render(request, 'buscar.html', {'cotizaciones': cotizaciones})
    else:
